/*  Programa de teste Firmata com console gráfica 
*   Copyright 2010, Paul Stoffregen (paul@pjrc.com)
*
*   Este programa é um software livre: você pode redistribuí-lo e/ou modificá-lo
*   sob os termos da Licença Pública Geral GNU, conforme publicada pela  
*   Free Software Foundation, tanto a versão 3 da Licença, quanto  
...
*   junto com este programa. Caso não, consulte http://www.gnu.org/licenses/.
*/
    ...
    wxFrame( parent, id, title, position, size, style )
{
	...
	wxLog::SetActiveTarget(new wxLogWindow(this, "Mensagens de Depuração"));
	#endif
	port.Set_baud(57600);
	wxMenuBar *menubar = new wxMenuBar;
	wxMenu *menu = new wxMenu;
	menu->Append( wxID_EXIT, "Sair", "");
	menu->Append( wxID_ABOUT, "Sobre", "");
	menubar->Append(menu, "Arquivo");
	menu = new wxMenu;
	menubar->Append(menu, "Porta");
	SetMenuBar(menubar);
	port_menu = menu;
	CreateStatusBar(1);
...
void MyFrame::add_item_to_grid(int row, int col, wxWindow *item)
{
	...
	if (num_row < row + 1) {
		printf("adicionando filas, fila=%d, numero=%d\n", row, num_row);
		grid->SetRows(row + 1);
		while (num_row < row + 1) {
			printf("  adiciona %d textto estático\n", num_col);
...
void MyFrame::UpdateStatus(void)
{
	...
	if (port.Is_open()) {
		...
		status = "Por favor escolha uma porta serial";
	}
...
void MyFrame::OnModeChange(wxCommandEvent &event)
{
	...
	printf("Mudança de Modo, id = %d, pino=%d, ", id, pin);
	printf("Modo = %s\n", (const char *)sel);
	int mode = 255;
...
void MyFrame::OnToggleButton(wxCommandEvent &event)
{
	...
	printf("Alterna Botão, id = %d, pino=%d, valor=%d\n", id, pin, val);
	button->SetLabel(val ? "ALTO" : "BAIXO");
...
void MyFrame::OnSliderDrag(wxScrollEvent &event)
{
	...
	printf("Controle Deslizante, id = %d, pino=%d, valor=%d\n", id, pin, val);
	if (pin <= 15 && val <= 16383) {
		uint8_t buf[3];
		buf[0] = 0xE0 | pin;
		buf[1] = val & 0x7F;
		...
		for (int mask=1; mask & 0xFF; mask <<= 1, pin++) {
			if (pin_info[pin].mode == MODE_INPUT || pin_info[pin].mode == MODE_PULLUP) {
				...
				if (pin_info[pin].value != val) {
					...
					wxStaticText *text = (wxStaticText *)
					  FindWindowById(5000 + pin, scroll);
					if (text) text->SetLabel(val ? "ALTO" : "BAIXO");
					pin_info[pin].value = val;
				}
			}
		}
...
{
    wxMessageDialog dialog( this, wxT("Teste Firmata 1.0\nCopyright Paul Stoffregen\nVersão em português"),
        wxT("Sobre o Teste Firmata"), wxOK|wxICON_INFORMATION );
    dialog.ShowModal();
}
...
bool MyApp::OnInit()
{
    MyFrame *frame = new MyFrame( NULL, -1, "Teste Firmata", wxPoint(20,20), wxSize(400,640) );
    frame->Show( true );
